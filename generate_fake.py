...
def main(opt):
    ...
    with open(text_corpus, 'r') as _f:
        ...
        texts = line.split()  # 按空格分割成单词列表

    if opt.generate_type == 'test_fake':
        target_dir = os.path.join('data/output', line)
    else:
        target_dir = os.path.join(opt.save_dir, opt.generate_type)
    new_path = './data/style_samples_new'
    """Setup data_loader instances"""
    # train_wid = ['292', '293', '295']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--one_dm', dest='one_dm', default='model_zoo/One-DM'
                                                           '-ckpt.pt', help='Pre-train model for generating')
    parser.add_argument('--generate_type', dest='generate_type', required=True,
                        help='Four generation settings: iv_s, iv_u, oov_s, oov_u, test_fake')
