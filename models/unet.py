...
import math
import random
from models.fusionAuto import AutoregressiveHandwritingGenerator
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.Mix_net = AutoregressiveHandwritingGenerator(
            style_in_chans=512,       # 风格特征提取器 (ResNet) 输出的通道数
            content_char_dim=512,     # 字符图像编码器输出的维度
            depth=12,                 # 自回归 Transformer 的层数
            embed_dim=768,            # Transformer 内部的工作维度
            num_heads=12,             # 注意力头的数量
            output_dim=context_dim,   # 最终输出维度，必须与 U-Net 的 context_dim 匹配
            contrastive_dim=256       # 对比学习嵌入的维度
        )
    ...
    def forward(self, x, timesteps=None, style=None, content=None, tag='test', **kwargs):
        """
        ...
        :return: an [N x C x ...] Tensor of outputs.
        style torch.Size([96, 2, 64, 352])
        content torch.Size([96, 9, 16, 16])
        Mix_net->context torch.Size([96, 9, 512])
        """
        #print('y', y.shape)
        ...
        if tag=='train':
            context, low_nce_emb = self.Mix_net(style, content)
            # context1, low_nce_emb1 = self.mix_net(style, content)
        else:
            context = self.Mix_net.generate(style, content)
            # context1 = self.mix_net.generate(style, content)
        h = x.type(self.dtype)
