...
class Diffusion:
    ...
    def train_ddim(self, model, x, styles, content, total_t, sampling_timesteps=50, eta=0):
        ...
        for time, time_next in tqdm(time_pairs, position=1, leave=False, desc='sampling'):
            ...
            predicted_noise, low_nce_emb = model(x, time, styles, content, tag='train')
        ...
        return x, noise_list[0], low_nce_emb
    ...
    #     return x
    def ddim_sample(
            self, model, vae, n, x, styles, content,
            sampling_timesteps=50, eta=0,
            save_intermediates=False,  # 是否保存中间结果
            save_dir=None  # 保存目录
    ):
        """
        DDIM 采样，同时支持保存中间去噪图像
        """
        import os, torchvision
        model.eval()

        if save_intermediates:
            assert save_dir is not None, "保存中间结果需要提供 save_dir"
        ...
        for step_idx, (time, time_next) in enumerate(tqdm(time_pairs, position=1, leave=False, desc='sampling')):

            time_tensor = (torch.ones(n) * time).long().to(self.device)
            time_next_tensor = (torch.ones(n) * time_next).long().to(self.device)

            predicted_noise = model(x, time_tensor, styles, content)

            beta = self.beta[time_tensor][:, None, None, None]
            alpha_hat = self.alpha_hat[time_tensor][:, None, None, None]
            alpha_hat_next = self.alpha_hat[time_next_tensor][:, None, None, None]

            x_start = (x - (1 - alpha_hat).sqrt() * predicted_noise) / (alpha_hat.sqrt())

            if save_intermediates and (step_idx % 2 == 0 or step_idx == len(time_pairs)-1):
                with torch.no_grad():
                    lat = 1 / 0.18215 * x_start
                    inter = vae.decode(lat).sample
                    inter = (inter / 2 + 0.5).clamp(0, 1).cpu()  # 归一化到 [0,1]
                    ...
                    for i in range(inter.shape[0]):
                        img = inter[i]  # [C,H,W]
                        pil_img = torchvision.transforms.ToPILImage()(img).convert("L")
                        pil_img.save(os.path.join(step_dir, f"{i}.png"))

            if time_next_tensor[0] < 0:
                x = x_start
                continue
        ...
        latents = 1 / 0.18215 * x
        image = vae.decode(latents).sample
        image = (image / 2 + 0.5).clamp(0, 1).cpu().permute(0, 2, 3, 1).contiguous().numpy()
        image = torch.from_numpy(image)
        x = image.permute(0, 3, 1, 2).contiguous()
