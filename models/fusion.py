...
class Mix_Tr(nn.Module):
    ...
    def forward(self, style, content):
        """
            ...
            content: torch.Size([8, 8, 16, 16])
            content after rearrange: torch.Size([64, 1, 16, 16])
            content after content_encoder: torch.Size([64, 512, 1, 1])
            content after rearrange: torch.Size([8, 8, 512])
            content after add_position1D: torch.Size([8, 8, 512])
            anchor_low_feature: torch.Size([72, 8, 512])
            hs after fusion: torch.Size([1, 8, 8, 512])
        """
        # get and style feature
        ...
        # print("content:", content.shape, anchor_low_feature.shape)
        # print("anchor_low_feature:", anchor_low_feature.shape)
        ...
        hs = self.decoder(content, anchor_low_feature, tgt_mask=None)
        # print("hs after fusion:", hs.shape)
        style_hs = hs[0].permute(1, 0, 2).contiguous()

        return style_hs, low_nce_emb  # n t c
